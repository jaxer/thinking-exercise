...
from src.http_request import http_request, BadReplyError
...
class DecoupledRequest(object):
    ...
    def do_request(self):
        ...
        d = http_request(self.create_url(), method='GET')
        d.addCallback(self.on_request_success)
        d.addErrback(self.on_request_error)
        return d

    def on_request_success(self, body):
        self.log('request successful. waiting for callback')

    def on_request_error(self, failure):
        self.log('request error')

        if not self.client_request.finished:
            self.client_request.setResponseCode(502 if isinstance(failure.value, BadReplyError) else 503)
            self.client_request.write("Backend failed with: %s" % failure.getErrorMessage())
